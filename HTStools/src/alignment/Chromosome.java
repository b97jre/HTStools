...
public class Chromosome implements Serializable{
	...
	public void compareDistribution(Chromosome otherRun,ExtendedWriter ER){
	...
	private void readChromosomeSequence(ExtendedReader ER){
		...
		ER.skipLine();
//		System.out.println("L�ser in kromosom sekvensen");
		while(ER.more()){
...
//		System.out.println("Klar");
	...
	public void print3UTRs(ExtendedWriter EW, int length){
		System.out.println("Chromosome "+this.Name);
		for(int i = 0; i < codingGenes.size();i++){
			codingGenes.get(i).print3UTR(this, EW, length);
		}
	}


	public void print3UTRsTab(ExtendedWriter EW, int length){
		System.out.println("Chromosome "+this.Name);
		for(int i = 0; i < codingGenes.size();i++){
			codingGenes.get(i).print3UTRtab(this, EW, length);
		}
	}
...
//	
	public void printSurrounding(ExtendedWriter EW, int surrounding){
		for(int i = 0; i < this.ncRNAs.size(); i++){
			if(this.ncRNAs.get(i).getNrOfHits() > 0 ){
					this.ncRNAs.get(i).printSurrounding(this, EW,surrounding);
			}
		}
		for(int i = 0; i < this.repeats.size(); i++){
			if(this.repeats.get(i).getNrOfHits() > 0 ){
				this.repeats.get(i).printSurrounding(this, EW,surrounding);
			}
		}
		
		for(int i = 0; i < this.intergenicRegions.size(); i++){
			if(this.intergenicRegions.get(i).getNrOfHits() > 0 ){
				this.intergenicRegions.get(i).printSurrounding(this, EW,surrounding);
			}
		}
		for(int i = 0; i < this.codingGenes.size(); i++){
			if(this.codingGenes.get(i).getNrOfHits() > 0 ){
				this.codingGenes.get(i).printSurrounding(this, EW,surrounding);
			}
		}
	}
	...
	public int[] DistributionSolidSequence(int[] distribution){
		for(int i = 0; i < this.ncRNAs.size(); i++){
			distribution[0] += this.ncRNAs.get(i).getNrOfHits();
		}
		for(int i = 0; i < this.repeats.size(); i++){
			distribution[1] += this.repeats.get(i).getNrOfHits();
		}
		
		for(int i = 0; i < this.intergenicRegions.size(); i++){
			distribution[2] += this.intergenicRegions.get(i).getNrOfHits();
		}
		
		for(int i = 0; i < this.codingGenes.size(); i++){
			distribution[3] += this.codingGenes.get(i).getNrOfHits(); 
		}
