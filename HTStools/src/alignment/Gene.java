...
public class Gene extends Object implements Serializable {
	...
	public void printmRNA(ExtendedWriter EW, int[] sequence,String contigName,Hashtable <Integer,StructuralVariation> SVs){
	...
	public void printPersonalmRNA(ExtendedWriter[] EWs, int[] sequence,String contigName,Hashtable <Integer,
			StructuralVariation> SVs,ArrayList<Integer> SVorder, String[] sample){
	...
	public int[] getNumberOfHeterozygousSites(Hashtable <Integer,
		...
		while(stopPointer < SVorder.size() && SVorder.get(stopPointer)<= this.right) stopPointer++;

		int[] heterozygousSites = new int[samples.size()];
		for(int i = 0; i < heterozygousSites.length; i++){
			heterozygousSites[i] = 0;
		}
		for(int i = startPointer; i < stopPointer; i++){
			heterozygousSites = SVs.get(SVorder.get(i)).addHeterozygousSites(heterozygousSites,samples);
		}
		...
		if(this.blastHits != null){
			...
			for(int i = 0; i < blastHits.size(); i++){
				...
				if(before > -1 && blastHits.get(i).getScore()+ blastHits.get(before).getScore()-beforeDist*penalty > Math.max(blastHits.get(i).getScore(), blastHits.get(before).getScore())){
					...
					//					System.out.println();
					//					blastHits.get(i).printHit();
					//					blastHits.get(before).printHit();
				...
				if(before != -1 || after != -1){
					//					System.out.println(i+" "+before+" "+after);
					int k = 0;
					while(blastHits.get(i).getHitName().compareTo(blastHits.get(k).getHitName()) != 0)k++;
					i=k;
					i--;
				}
	...
	/*	public void keepOnlyBestReciprocalHits(Hashtable<String,Gene> Genes){

		if(this.blastHits != null){
			int bestFound = -1;
			int pointer = this.getBestHitPointer();
			String query = blastHits.get(pointer).getQueryName();
			String hitName = blastHits.get(pointer).getHitName();
			String otherBest = Genes.get(hitName).getBestHit();
			if(query.compareTo(otherBest) == 0){
				bestFound = pointer;
			}else{
				blastHits.remove(pointer);
			} 
			if(bestFound > -1){
				BlastHit BH = blastHits.get(bestFound);
				for(int i = 0; i < blastHits.size(); i++){
					blastHits.remove(i);
					i--;
				}
	...
	public void mergeBlastHitsTotal(ExtendedWriter EW){
		int[] covered = new int[this.length];
		...
		if(this.blastHits != null){
			//System.out.println(this.Name);
			for(int i = 0; i < blastHits.size(); i++){
				if(blastHits.get(i).getQueryStart()>blastHits.get(i).getQueryStop() ){
					for(int j = blastHits.get(i).getQueryStop(); j <= blastHits.get(i).getQueryStart();j++)covered[j-1]=1;
					if(blastHits.get(i).getQueryStart()-blastHits.get(i).getQueryStop()>bestBlastHit)bestBlastHit= blastHits.get(i).getQueryStart()-blastHits.get(i).getQueryStop();
				}
				else{
					for(int j = blastHits.get(i).getQueryStop(); j >= blastHits.get(i).getQueryStart();j--)covered[j-1]=1;
					if(blastHits.get(i).getQueryStop()-blastHits.get(i).getQueryStart() > bestBlastHit)bestBlastHit= blastHits.get(i).getQueryStop()-blastHits.get(i).getQueryStart();
			...
			int fractions = 0;
			int totalCoverage = 0;
			int longestCoverage = 0;
			int tempCoverage = 0;
			boolean C = false;
			for(int i = 0; i < covered.length; i++){
				if(covered[i]==1){
					totalCoverage++;
					if(C){// already in coverage
						tempCoverage++;
					}
					else{//first that is covered
						tempCoverage=1;
						C=true;
						fractions++;
					}
					...
					if(C){// already in coverage
						C=false;
						if(tempCoverage > longestCoverage){
							longestCoverage = tempCoverage;
						}
			...
			EW.println(this.Name+"\t"+this.length+"\t"+totalCoverage+"\t"+longestCoverage+"\t"+fractions+"\t"+bestBlastHit);
		}
		else{
			EW.println(this.Name+"\t"+this.length+"\t0\t0\t0\t0");
		}
	...
	public void printBestHit(ExtendedWriter EW, Hashtable<String,Gene> HT ){
		if(this.blastHits!= null){
			double score = getHighestScore();
			int pointer = 0;
			boolean found = false;
	...
	public void printBestCoverageMerged(ExtendedWriter EW){
		if(this.blastHits!= null){
